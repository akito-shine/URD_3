...
from gpu_utils import xp, to_cpu
import numpy as np
...
class Canvas:
    def __init__(self, placedb:ClusterDB, grid=224, fixed_nodes=False):
        ...
        self.overlap = xp.zeros((grid, grid), dtype=xp.float32)
        self.rudy = xp.zeros((grid, grid), dtype=xp.float32)
        self.hpwl = 0
    ...
    def get_hpwl_increment(self, cluster_id):
        hpwl_inc = xp.zeros((self.grid, self.grid), dtype=xp.float32)
        for pin in self.placedb.cluster_info[cluster_id]['pins']:
            if pin not in self.placedb.pin_info:
                continue
            for net in self.placedb.pin_info[pin]['nets']:
                if net in self.net_bounds:
                    ...
                    y_max = self.net_bounds[net]['y_max']
                    if x_min - offset_x > 0:
                        vec = xp.arange(x_min-offset_x, 0, -1, dtype=xp.float32)
                        if x_min - offset_x  > self.grid:
                            vec = vec[:self.grid]
                        hpwl_inc[0 : min(x_min-offset_x, self.grid) :] += xp.tile(vec[:, xp.newaxis],(1, self.grid))
                    if x_max - offset_x < self.grid:
                        vec = xp.arange(0, self.grid + offset_x - x_max, dtype=xp.float32)
                        if x_max - offset_x < 0:
                            vec = vec[-x_max + offset_x:]
                        hpwl_inc[max(x_max-offset_x, 0) : self.grid, :] += xp.tile(vec[:, xp.newaxis],(1, self.grid))
                    if y_min - offset_y > 0:
                        vec = xp.arange(y_min-offset_y, 0, -1, dtype=xp.float32)
                        if y_min - offset_y > self.grid:
                            vec = vec[:self.grid]
                        hpwl_inc[:, 0 : min(y_min-offset_y, self.grid)] += xp.tile(vec[xp.newaxis, :], (self.grid, 1))
                    if y_max - offset_y < self.grid:
                        vec = xp.arange(0, self.grid + offset_y - y_max, dtype=xp.float32)
                        if y_max - offset_y < 0:
                            vec = vec[-y_max + offset_y:]
                        hpwl_inc[:, max(y_max-offset_y, 0) : self.grid] += xp.tile(vec[xp.newaxis, :], (self.grid, 1))
        return hpwl_inc
    
    def get_pos(self, cluster_id):
        pos = xp.zeros((self.grid, self.grid), dtype=xp.float32)
    ...
    def get_overlap(self, cluster_id):
        overlap = xp.zeros((self.grid, self.grid), dtype=xp.float32)
        w = ceil(self.placedb.cluster_info[cluster_id]['x'] / self.ratio)
        h = ceil(self.placedb.cluster_info[cluster_id]['y'] / self.ratio)
        grid = xp.arange(self.grid, dtype=xp.float32)
        for cluster_id1 in self.node_pos:
            ...
            left_max = xp.maximum(x_left, grid)
            right_min = xp.minimum(x_right, grid + w)
            overlap_x = xp.maximum(right_min - left_max, 0)

            left_max = xp.maximum(y_left, grid)
            right_min = xp.minimum(y_right, grid + h)
            overlap_y = xp.maximum(right_min - left_max, 0)

            overlap += overlap_x[:, xp.newaxis] * overlap_y[xp.newaxis, :]

        overlap[self.grid-w+1:self.grid, :] = xp.inf
        overlap[:, self.grid-h+1:self.grid] = xp.inf
        return overlap
from placedb import ClusterDB
from math import ceil
import numpy as np
